...
export const Route = createFileRoute("/confirmation")({
  ...
  head: () => ({
    meta: [
      { title: "Order Confirmed — Lumina Beauty" },
      { name: "description", content: "Your Lumina Beauty order has been confirmed. Thank you for choosing luxury natural skincare." },
    ],
  }),
});
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <div
        ...
        style={{ background: "radial-gradient(ellipse at 50% 40%, rgba(139,47,78,0.07) 0%, rgba(232,180,184,0.04) 40%, transparent 70%)" }}
        aria-hidden
      />
      ...
      <div className="relative z-10 max-w-2xl mx-auto text-center py-20">
        ...
        <div className="mx-auto mb-8 w-24 h-24 rounded-full bg-surface border border-border flex items-center justify-center rose-gold-glow">
          <svg className="w-12 h-12 text-rose-gold" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24" strokeLinecap="round" strokeLinejoin="round">
            <path d="M20 6 9 17l-5-5" />
          </svg>
        </div>

        <span className="text-[10px] uppercase tracking-[0.4em] text-rose-gold">Order Confirmed</span>

        <h1 className="mt-4 font-display leading-[0.95] text-ivory" style={{ fontSize: "clamp(2.5rem, 7vw, 5rem)" }}>
          Thank You<br />
        ...
        <p className="mt-6 text-muted-foreground leading-relaxed max-w-md mx-auto">
          Your Lumina Beauty order has been placed successfully. A confirmation email is on its way to your inbox.
        </p>
        ...
        <div className="mt-10 glass-card rounded-3xl p-8 text-left space-y-5">
          <div className="flex justify-between items-center text-sm border-b border-border pb-4">
            <span className="text-muted-foreground uppercase tracking-widest text-[10px]">Order Number</span>
            <span className="font-display text-champagne text-lg">{orderNumber}</span>
          </div>
          <div className="flex justify-between items-center text-sm border-b border-border pb-4">
            <span className="text-muted-foreground uppercase tracking-widest text-[10px]">Estimated Delivery</span>
          ...
          <div className="flex justify-between items-center text-sm">
            <span className="text-muted-foreground uppercase tracking-widest text-[10px]">Total</span>
        ...
        <div className="mt-8 grid grid-cols-3 gap-4">
          {[
            ...
            <div key={p.label} className="glass-card rounded-2xl p-4 text-center">
              ...
              <p className="mt-2 text-[9px] uppercase tracking-widest text-muted-foreground">{p.label}</p>
            </div>
          ))}
        </div>
