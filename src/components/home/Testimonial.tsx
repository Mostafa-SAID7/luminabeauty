  ...
  return (
    <section className="relative py-20 md:py-32 px-5 sm:px-6 lg:px-10 max-w-5xl mx-auto text-center">
      ...
      <Reveal variant="scale" delay={120} className="mt-12 md:mt-16 relative glass-card rounded-3xl p-8 sm:p-10 md:p-12 rose-gold-glow">
        ...
        <SmartImage src={TESTIMONIAL_DATA.img} alt={`${TESTIMONIAL_DATA.author}, verified Lumina Beauty customer from ${TESTIMONIAL_DATA.location}`} width={800} height={800} wrapperClassName="mx-auto w-16 h-16 sm:w-20 sm:h-20 rounded-full ring-2 ring-rose-gold" className="w-full h-full object-cover" />
        ...
        <div className="mt-6 sm:mt-8">
          <div className="font-display text-lg sm:text-xl text-ivory">{TESTIMONIAL_DATA.author}</div>
          <div className="text-[10px] uppercase tracking-[0.3em] text-muted-foreground mt-1">{TESTIMONIAL_DATA.location}</div>
        </div>
      </Reveal>
    </section>
  );
