  ...
  return (
    <section id="results" className="relative py-20 md:py-32 px-5 sm:px-6 lg:px-10 max-w-6xl mx-auto">
      <Reveal className="text-center max-w-2xl mx-auto mb-12 md:mb-16">
        ...
        <h2 className="mt-4 font-display text-ivory" style={{ fontSize: "clamp(2.25rem, 6vw, 4rem)" }}>{BEFORE_AFTER_DATA.title} <em className="shimmer-text not-italic italic font-light">{BEFORE_AFTER_DATA.titleEm}</em></h2>
        <p className="mt-5 md:mt-6 text-sm md:text-base text-muted-foreground">{BEFORE_AFTER_DATA.desc}</p>
      </Reveal>
      <Reveal variant="scale">
        ...
        >
          <img src={BEFORE_AFTER_DATA.after} alt="Lumina Radiance Serum after 14 days — visible glow and even skin tone" loading="lazy" width={1024} height={1280} className="absolute inset-0 w-full h-full object-cover pointer-events-none" />
          <div className="absolute inset-0 overflow-hidden pointer-events-none" style={{ width: `${pos}%` }}>
            <img src={BEFORE_AFTER_DATA.before} alt="Skin before using Lumina Radiance Serum — uneven tone and dullness" loading="lazy" width={1024} height={1280} className="absolute inset-0 h-full object-cover" style={{ width: `${(100 / pos) * 100}%`, maxWidth: "none" }} />
          </div>
          ...
          <span className="absolute bottom-6 left-6 text-[10px] uppercase tracking-[0.3em] glass-card rounded-full px-4 py-2 text-ivory pointer-events-none">{BEFORE_AFTER_DATA.labelBefore}</span>
          <span className="absolute bottom-6 right-6 text-[10px] uppercase tracking-[0.3em] bg-primary text-primary-foreground rounded-full px-4 py-2 pointer-events-none">{BEFORE_AFTER_DATA.labelAfter}</span>
      ...
      <p className="mt-6 text-center text-xs uppercase tracking-[0.3em] text-muted-foreground">{BEFORE_AFTER_DATA.product}</p>
    </section>
  );
}
